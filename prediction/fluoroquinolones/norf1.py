...
def main():
    ...
    smis = ['CCN1C=C(C(=O)C2=CC(=C(C=C12)N3CC[N+]([H])([H])CC3)F)C(=O)O' for _ in range(n)]
    smis += ['CCN1C=C(C(=[O+][H])C2=CC(=C(C=C12)N3CCNCC3)F)C(=O)O' for _ in range(n)]
    smis += ['CC[N+]1([H])C=C(C(=O)C2=CC(=C(C=C12)N3CCNCC3)F)C(=O)O' for _ in range(n)]
    smis += ['CCN1C=C(C(=O)C2=CC(=C(C=C12)[N+]3([H])CCNCC3)F)C(=O)O' for _ in range(n)]
    structures = []
    for i in range(1, n + 1):
        fname = 'norf_pA_{}.xyzmq'.format(i)
        with open(fname, 'r') as f:
            structures.append(f.read())
    for i in range(1, n + 1):
        fname = 'norf_pB_{}.xyzmq'.format(i)
        with open(fname, 'r') as f:
            structures.append(f.read())
    for i in range(1, n + 1):
        fname = 'norf_pC_{}.xyzmq'.format(i)
        with open(fname, 'r') as f:
            structures.append(f.read())
    for i in range(1, n + 1):
        fname = 'norf_pD_{}.xyzmq'.format(i)
        with open(fname, 'r') as f:
            structures.append(f.read())
    ...
    savetxt('norf_X_CUST.txt', X_cust)
    savetxt('norf_X_MQN.txt', X_mqn)
    savetxt('norf_X_MD3D.txt', X_md3d)
    savetxt('norf_X_COMB.txt', X_comb)
    """
